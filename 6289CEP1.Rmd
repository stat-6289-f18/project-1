    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  ...
                  tabPanel(title = "Linear Regression Analysis", 
                          ...
                          verbatimTextOutput(outputId = "loutput"))
      )
    )
  )
)
...
server <- function(input, output) {
  ...
  output$description <- renderText({
    ...
    avg_x <- years_selected() %>% pull(input$x) %>% mean() %>% round(2)
    paste("Average", input$x, "=", avg_x)
  })
  
  output$Avg_y <- renderText({
    avg_y <- years_selected() %>% pull(input$y) %>% mean(na.rm=TRUE) %>% round(2)
    paste("Average", input$y, "=", avg_y)
  })
  
     output$loutput <- renderPrint({
    x <- years_selected() %>% pull(input$x)
    y <- years_selected() %>% pull(input$y)
    su <- summary(lm(y ~ x, data = poll)) 
    print(su, digits = 4, signif.stars = TRUE)
  })
